;!function (w){
     ...
     * it is realized  methods of the WEB API: URLSearchParams. The IE's all version is unsupported Iterator, so keys() is returned an array object, entries（），values() methods respectively are returned an object;
    ...
    w.URLParams=function (s){
        ...
        s=s.indexOf('?')>-1? s.substr(s.indexOf('?')+1):s;
         paramString=s.split('&');
        var param={};
        for (var pp=0;pp<paramString.length;pp++ ){
